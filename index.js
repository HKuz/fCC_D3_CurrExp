...
Promise.all([getJSONData, getCSVData]).then(function(values) {
  ...
  const low = d3.min(+population.Population);  // d3.min(popArray);
  const high = d3.max(+population.Population)  // d3.max(popArray);
  console.log("Min population is: " + low);  // 12,876 -> Nauru
  console.log("Max population is: " + high);  // 1,386,395,000 -> China
  console.log(d3.extent(+population.Population));  // [12876, 1386395000]
